...
def test_trace_multiple_runs(tmp_path):
    class A(Task):
        def run_step(self) -> int:
            return 1

    a = A()
    trace = tmp_path / "multi.log"
    wf = Workflow(outputs=[a], trace=str(trace))
    wf.run({})
    wf.run({})

    lines = trace.read_text().splitlines()
    entries = [json.loads(line) for line in lines]
    starts = [e for e in entries if e["event"] == "workflow_started"]
    finishes = [e for e in entries if e["event"] == "workflow_finished"]

    assert len(starts) == 2
    assert len(finishes) == 2
    assert starts[0]["workflow_id"] == starts[1]["workflow_id"]
    assert starts[0]["workflow_instance_id"] != starts[1]["workflow_instance_id"]
